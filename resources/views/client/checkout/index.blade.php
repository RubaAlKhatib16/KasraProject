...
<div class="checkout-page">
    <div class="container">
        ...
        <div class="checkout-grid">
            ...
            <form method="POST" action="{{ route('checkout.store') }}" enctype="multipart/form-data" class="checkout-form" id="checkoutForm">
                @csrf
                ...
                <div class="form-section">
                    ...
                    <div class="form-row">
                        <div class="form-group" id="nationalIdGroup">
                            ...
                            <input type="text" name="national_id" id="nationalIdInput"
                                   maxlength="10" inputmode="numeric" pattern="\d{10}"
                                   placeholder="أدخل 10 أرقام" required>
...
@push('styles')
<style>
.checkout-page { padding: 2rem 0; min-height: calc(100vh - 200px); }
...
.section-title {
    ...
    border-right: 3px solid #FF4F8B; padding-right: 1rem;
}
...
.form-group label {
    display: flex; align-items: center; gap: 0.5rem;
    font-size: 0.85rem; font-weight: 600; color: #E9B3FB; margin-bottom: 0.4rem;
}
.form-group input,
.form-group select,
.form-group textarea {
...
/* رقم الهوية */
#nationalIdInput { letter-spacing: 0.15em; font-size: 1rem; font-weight: 600; }
#nationalIdInput.is-valid  { border-color: #4ADE80 !important; box-shadow: 0 0 0 3px rgba(74,222,128,0.15) !important; }
#nationalIdInput.is-invalid{ border-color: #FF6B6B !important; box-shadow: 0 0 0 3px rgba(255,107,107,0.15) !important; }
.id-feedback { display: block; font-size: 0.75rem; font-weight: 600; margin-top: 0.35rem; min-height: 1rem; padding-right: 0.25rem; }
.id-feedback.success { color: #4ADE80; }
.id-feedback.error   { color: #FF6B6B; }
...
/* معاينة البطاقة */
.card-preview-wrapper { display: flex; justify-content: center; margin-bottom: 1.8rem; }
...
.credit-card:hover { transform: translateY(-4px) rotateX(3deg); }
...
.chip-lines::before, .chip-lines::after {
    content: '';
    position: absolute; background: rgba(0,0,0,0.25);
}
.chip-lines::before { top: 50%; left: 0; right: 0; height: 1px; transform: translateY(-50%); }
.chip-lines::after  { left: 50%; top: 0; bottom: 0; width: 1px; transform: translateX(-50%); }

.card-network { font-size: 2.2rem; color: white; opacity: 0.9; }
...
.card-label { font-size: 0.55rem; color: rgba(255,255,255,0.5); text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 2px; }
.card-holder-display { font-size: 0.85rem; color: white; font-weight: 600; letter-spacing: 0.05em; text-transform: uppercase; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.card-expiry-display  { font-size: 0.85rem; color: white; font-weight: 600; font-family: 'Courier New', monospace; }

/* حقول البطاقة */
.card-input-wrapper { position: relative; }
.card-input-wrapper input { padding-left: 3rem !important; }
.card-type-icon, .cvv-toggle {
    position: absolute; left: 1rem; top: 50%; transform: translateY(-50%);
    color: #FFB3C7; font-size: 1.2rem; pointer-events: none;
}
.cvv-toggle { pointer-events: all; cursor: pointer; font-size: 1rem; }

#cardNumber { font-family: 'Courier New', monospace; letter-spacing: 0.12em; font-size: 1rem; }
#cardExpiry { font-family: 'Courier New', monospace; letter-spacing: 0.1em; }
#cardCvv    { font-family: 'Courier New', monospace; letter-spacing: 0.15em; }
...
.security-bar {
    ...
    color: #86EFAC; font-size: 0.8rem; margin-top: 1rem;
}
.security-bar i:first-child { color: #4ADE80; font-size: 1rem; }
.security-bar span { flex: 1; }
.security-logos { display: flex; gap: 0.5rem; font-size: 1.5rem; color: white; opacity: 0.6; }

.cvv-hint { cursor: help; color: #94A3B8; font-size: 0.8rem; margin-right: 4px; }

/* ملف البطاقة */
.file-upload-wrapper { display: flex; align-items: center; gap: 1rem; flex-wrap: wrap; }
.file-input { display: none; }
.file-label { background: rgba(255,79,139,0.2); border: 1px solid rgba(233,179,251,0.5); border-radius: 60px; padding: 0.6rem 1.2rem; cursor: pointer; color: #FFB3C7; transition: background 0.2s; }
.file-label:hover { background: rgba(255,79,139,0.4); }
.file-name { color: #94A3B8; font-size: 0.8rem; }
.image-preview { margin-top: 1rem; max-width: 200px; border-radius: 16px; overflow: hidden; display: none; }
.image-preview img { width: 100%; }
.form-hint { color: #A5D6A7; font-size: 0.7rem; }

/* أزرار */
.form-actions { display: flex; gap: 1rem; margin-top: 1rem; }
.btn-submit {
    background: linear-gradient(105deg, #FF4F8B, #E6497D);
    border: none; padding: 0.9rem 2rem; border-radius: 60px;
...
.btn-back {
    background: transparent; border: 1px solid rgba(233,179,251,0.6);
    padding: 0.9rem 2rem; border-radius: 60px; color: #FFB3C7;
    text-decoration: none; display: inline-flex; align-items: center; gap: 0.6rem;
}

/* ملخص الطلب */
.order-summary {
    background: rgba(20, 25, 40, 0.7); backdrop-filter: blur(12px);
    border-radius: 32px; padding: 1.8rem;
    border: 1px solid rgba(233,179,251,0.25);
    height: fit-content; position: sticky; top: 2rem;
}
.summary-items { margin: 1.5rem 0; padding: 1rem 0; border-top: 1px solid rgba(233,179,251,0.2); border-bottom: 1px solid rgba(233,179,251,0.2); }
.summary-item  { display: flex; justify-content: space-between; padding: 0.6rem 0; color: #CBD5E1; }
.item-info     { display: flex; gap: 0.8rem; }
.summary-total { display: flex; justify-content: space-between; font-size: 1.2rem; font-weight: 800; margin-top: 1rem; color: white; }
.total-price   { color: #FF4F8B; }
.summary-note  { margin-top: 1rem; color: #64748B; font-size: 0.8rem; display: flex; align-items: center; gap: 0.4rem; }
...
function validateJordanianNationalId(id) {
    ...
    if (sum % 10 !== 0) return { valid: false, message: 'رقم الهوية غير صحيح، يرجى المراجعة' };
